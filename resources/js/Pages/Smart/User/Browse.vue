...
const handleConfirmAddToCart = () => {
  ...
  router.post(route(routeName), {
    ...
    onSuccess: () => {
      ...
      const cartName = isConsumable ? t('requests.consumableCartTitle') : t('requests.borrowCartTitle');
      const cartUrl = isConsumable ? route('smart.asset-cart') : route('smart.borrow-cart');

      toast.success(
        h('span', [
          t('requests.addedToCartSuccess', { count: quantity.value, cart: '' }),
          h('a', {
            href: cartUrl,
    ...
    onError: (errors) => {
      ...
      const msg = Array.isArray(firstError) ? firstError[0] : (firstError || t('common.error'));
...
<template>
  <AppLayout :title="$t('requests.browseItems')">
    <div class="space-y-6">
      <div>
        <h1 class="text-lg font-bold text-gray-900 leading-none mb-5">{{ $t('requests.browseItems') }}</h1>
        ...
        <div class="space-y-3 mb-5">
          <div class="flex flex-col sm:flex-row sm:items-end gap-4">
            ...
            <div class="space-y-1.5 w-full sm:max-w-sm">
              <label class="text-xs text-muted-foreground font-medium block ml-0.5">{{ $t('common.search') }}</label>
              <TableSearch 
                v-model="searchQuery" 
            ...
            <div class="space-y-1.5 w-full sm:w-auto flex-1">
              <label class="text-xs text-muted-foreground font-medium block ml-0.5">{{ $t('requests.filterAndSort') }}</label>
              <div class="grid grid-cols-[1fr_1fr_auto] sm:flex sm:items-center gap-2 sm:gap-3">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" :class="['w-full sm:w-[11.25rem] md:w-[12.5rem] justify-between rounded-[0.875rem] font-normal bg-white cursor-pointer', selectedCategory === 'all' ? 'text-muted-foreground' : 'text-foreground']">
                      <span class="truncate">{{ selectedCategory === 'all' ? $t('requests.allCategories') : selectedCategory }}</span>
                      <ChevronDown class="w-4 h-4 opacity-50 shrink-0" />
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent class="w-[12.5rem] rounded-[0.875rem]" align="start" :side-offset="4">
                    <DropdownMenuItem @select="selectedCategory = 'all'">{{ $t('requests.allCategories') }}</DropdownMenuItem>
                    <DropdownMenuItem 
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" class="w-full sm:w-[11.25rem] md:w-[12.5rem] justify-between rounded-[0.875rem] font-normal bg-white cursor-pointer text-foreground">
                      <span class="truncate">{{ selectedSort === 'asc' ? $t('requests.sortAZ') : $t('requests.sortZA') }}</span>
                      <ChevronDown class="w-4 h-4 opacity-50 shrink-0" />
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent class="w-[12.5rem] rounded-[0.875rem]" align="start" :side-offset="4">
                    <DropdownMenuItem @select="selectedSort = 'asc'">{{ $t('requests.sortAZ') }}</DropdownMenuItem>
                    <DropdownMenuItem @select="selectedSort = 'desc'">{{ $t('requests.sortZA') }}</DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>

                <!-- Clear Filter Button -->
                <Button variant="destructive" @click="clearFilter" class="hover:opacity-70 rounded-[0.875rem] px-0 sm:px-6 font-semibold text-white w-9 h-9 sm:w-auto sm:h-auto flex items-center justify-center shrink-0 cursor-pointer">
    ...
    <Dialog :open="isModalOpen" @update:open="val => { if (!isSubmitting) isModalOpen = val }">
      <DialogContent class="sm:max-w-[62.5rem] rounded-[0.875rem] bg-card shadow-2xl p-0 gap-0 border border-border overflow-hidden" :show-close-button="false">
        ...
        <div class="flex items-center justify-between pt-3 pb-2 px-4 border-b border-border">
          <div>
            <DialogTitle class="text-lg font-bold text-foreground">{{ t('requests.addToCart') }}</DialogTitle>
            <DialogDescription class="sr-only">
              {{ t('requests.addToCartDesc') }}
            </DialogDescription>
          </div>
        ...
        <div v-if="selectedProduct">
          ...
          <div class="px-4 sm:px-6 py-3 overflow-y-auto max-h-[70vh] space-y-6">
            <!-- Selected Item -->
            <div class="space-y-1">
              <span class="text-sm font-medium text-foreground block">{{ t('requests.selectedItem') }}</span>
              <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4">
                ...
                <div class="flex flex-col justify-center text-center sm:text-left">
                  <template v-if="!selectedBarangId">
                    ...
                    <p class="text-xs text-muted-foreground italic">{{ t('requests.photoIllustration') }}</p>
                  </template>
            ...
            <div class="space-y-1.5">
              <span class="text-sm font-medium text-foreground block">{{ t('requests.selectVariant') }} <span class="text-rose-500">*</span></span>
            ...
            <div class="space-y-1.5 pb-3">
              <label class="text-sm font-medium text-foreground block">
                {{ t('requests.requestedQuantity') }} <span class="text-rose-500">*</span>
              </label>
              <div class="flex items-center gap-3">
                <NumberField v-model="quantity" :min="1" :max="999999" :locale="locale === 'en' ? 'en-US' : 'id-ID'" class="w-32">
                  <NumberFieldContent>
                    <NumberFieldDecrement />
                    <NumberFieldInput />
                    <NumberFieldIncrement />
          ...
          <div class="py-4 px-4 sm:px-6 border-t border-border flex flex-col-reverse sm:flex-row sm:items-center justify-between gap-3">
            <p class="text-sm text-rose-500 italic font-medium">{{ t('requests.requiredField') }}</p>
            <div class="flex flex-row items-center gap-2 sm:gap-3 w-full sm:w-auto">
              ...
              >
                ...
                <span v-else>{{ t('requests.addToCart') }}</span>
              </Button>
            </div>
          </div>
        </div>
