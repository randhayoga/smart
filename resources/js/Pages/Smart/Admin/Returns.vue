...
onMounted(() => {
  if (dataTableRef.value?.table && rowsPerPage.value === 'Semua baris') {
    dataTableRef.value.table.setPageSize(999999);
  }
});
</script>
    ...
    <div class="space-y-4">
      <div class="px-4 bg-card rounded-xl border border-border shadow-sm overflow-hidden">
        <div class="py-5 no-print">
          <h2 class="text-lg font-bold text-foreground">Daftar Jadwal Pengembalian</h2>
        ...
        <div class="pb-4">
          ...
          <div v-if="searchQuery || timeFilter" class="print-only mb-4 text-left">
