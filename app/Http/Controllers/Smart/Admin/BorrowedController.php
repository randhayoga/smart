...
class BorrowedController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $borrowedList = $dbRequests->map(function ($req) {
            $daysLeft = '-';
            if ($req->end_date) {
                $now = Carbon::now();
            ...
            return [
                ...
                'dueDate' => $req->end_date ? $req->end_date->format('d-m-Y H:i') : '-',
                'daysLeft' => $daysLeft,
            ];
        });
    ...
    public function show(Request $request, $id): Response
    {
        ...
        // 5. Active / Loan duration
        $daysLeftText = '-';
        if ($reqObj->end_date) {
            $now = Carbon::now();
            $diff = $now->diffInDays($reqObj->end_date, false);
            $daysLeftText = $diff >= 0 ? "$diff hari tersisa" : "Terlambat " . abs($diff) . " hari";
        }
